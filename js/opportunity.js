$(document).ready(function(){	
	...
	var leadsRecords = $('#leadsListing').DataTable({
		...
		"order":[],
		"ajax":{
			url:"opportunity_action.php",
			type:"POST",
			data:{action:'listOpportunity'},
			dataType:"json"
		},
		"columnDefs":[
			{
	...
	$('#addOpportunity').click(function(){
		...
		$("#leadsModal").on("shown.bs.modal", function () { 
			$('#leadsForm')[0].reset();
			$('.modal-title').html("<i class='fa fa-plus'></i> Add Opportunity");			
			$('#action').val('addOpportunity');
			$('#save').val('Save');
		});
	});		
	...
	$(document).on('click', '.view', function(){
		var id = $(this).attr("id");
		var action = 'getOpportunityDetails';
		$.ajax({
			url:'opportunity_action.php',
			method:"POST",
			data:{id:id, action:action},
